...
def main():
    ...
    mbr_siz = st.number_input("Enter MBR Size", min_value=0.0, value=0.1)
    ...
    if st.button("Create Big Workload "):
        db = rt.db_list[keyd]
        db.createWorkLoad(tmaxd,range_freqd)
        dbr = rt.rtree_only_db_list[keyd]
        dbr.createWorkLoad(tmaxd,range_freqd)
        st.success("Workload ended successfully!")
        

    st.header("Benchmark")

    mbr_sizeb = st.number_input("Enter MBR Size for benchmark", min_value=0.0, value=0.1)
